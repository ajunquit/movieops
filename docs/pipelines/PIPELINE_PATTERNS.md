...
| Build Once, Deploy Many | ✅ | `deploy.yml` uses `az acr import` to copy the exact GHCR image built by `ci.yml` into the target environment's ACR — no rebuild; validated live in `dev` |
| Environment Promotion | 🟡 | `cd-dev.yml` / `cd-staging.yml` / `cd-production.yml` accept the same `image_tag`; `dev` is validated live, while staging/production remain pending |
...
| Automatic Rollback | 🟡 | `deploy.yml`: `kubectl rollout status` (health gate) + bounded smoke test against the Ingress IP; the successful path is validated live, while rollback to a previous revision remains to be exercised intentionally |
...
- **Two registries, two responsibilities.** CI publishes the immutable SHA-tagged artifact to GHCR using `GITHUB_TOKEN`; CD promotes that exact artifact server-side into the target environment's ACR with `az acr import`. There is no pull/push through the runner and no rebuild.
...
- **ACR is the runtime registry for Azure.** AKS pulls the promoted image from the environment's ACR through its Terraform-managed `AcrPull` role; GHCR remains the CI registry and source of promotion.
- **Sprint 8+9 completed a live end-to-end CD run.** The first attempt exposed TS-11 (the custom subnet NSG admitted Load Balancer probes but not real client traffic); after applying the Terraform rule through Genesis, both workloads reached `2/2`, the Ingress became reachable, and frontend/API returned HTTP 200.
